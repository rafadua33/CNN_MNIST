...
train_ann.py
============
Training Script for the ANN on MNIST

Trains the ANN defined in ann_model.py using mini-batch SGD with momentum,
then saves per-epoch metrics to results/ann_results.csv.
...
from ann_model import ANN, cross_entropy_loss
...
DATA_DIR     = '../data'
RESULTS_FILE = 'results/ann_results.csv'
...
def preprocess(images):
    """
    Normalise to [0, 1] and flatten from (N, 28, 28) to (N, 784).

    The ANN has no notion of spatial structure, so the 28x28 grid is
    unrolled into a single vector of 784 pixel values per image.
    """
...
def main():
    ...
    print(f"\nTraining ANN — {EPOCHS} epochs | batch={BATCH_SIZE} | lr={LEARNING_RATE}\n")
